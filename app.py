...
# 탭 2) 백테스트 요약: 목표연도 범위 × L=1..10 Heatmap(옵션) + 승률/평균R² 배지 + 최적L 분포 파이차트 + 연도별 비교표
...
with tab1:
    ...
    if perf.empty:
        ...
        win_text = "→ ‘최근 3년’이 최적." if best_L==3 else ""
        mode_text = {"all":"전월", "11-3":"난방월", "10-3":"난방확장"}[heating_mode]
        st.success(f"요약({mode_text}): **{target_year}년** 예측에서 **직전 {best_L}년 평균**이 최고(R²={best_R2:.4f}). {win_text}")
...
with tab2:
    ...
    with colC:
        show_heat = st.checkbox("Heatmap(Y×L)도 보기", value=True)
    ...
    fig_bar.update_layout(height=360, margin=dict(l=10,r=10,t=10,b=10), yaxis_range=[max(0.0, min(avg_recent, avg_long)-0.02), 1.0])
    st.plotly_chart(fig_bar, use_container_width=True)

    # 연도별 비교 표
    if not comp.empty:
        comp["최근(최적 L)"] = comp["Y"].apply(lambda Y: mat.query("Y==@Y and L<=3").sort_values("R2", ascending=False).iloc[0]["L"])
        comp["장기(최적 L)"] = comp["Y"].apply(lambda Y: mat.query("Y==@Y and L>=5").sort_values("R2", ascending=False).iloc[0]["L"])
    ...
    # 최적 L 분포 파이
    best_per_Y = mat.loc[mat.groupby("Y")["R2"].idxmax()][["Y","L"]].dropna()
    if not best_per_Y.empty:
        best_per_Y["구분"] = np.where(best_per_Y["L"]<=3, "최근(1–3년)",
                               np.where(best_per_Y["L"]>=5, "장기(5년+)", "중간(4년)"))
        dist = best_per_Y["구분"].value_counts().reindex(["최근(1–3년)","중간(4년)","장기(5년+)"]).fillna(0).reset_index()
        dist.columns = ["구분","연도수"]
